...
export async function fetchSpreadsheet(auth, spreadsheetId, ignorePatterns = []) {
    ...
    async function worker() {
        ...
        while (true) {
            ...
            const title = (_a = sheet.properties) === null || _a === void 0 ? void 0 : _a.title;
            if (!title) {
...
export async function generateLocalizationTable(sheets, { includeEmpty = false, } = {}) {
    ...
    for (const { title, values } of sheets) {
        ...
        for (let columnIndex = 3; columnIndex < header.length; columnIndex++) {
            ...
            const locale = localeTargets === null || localeTargets === void 0 ? void 0 : localeTargets[0];
            if (!locale) {
                continue;
            }
            ...
            log(`Sheet "${title}" has missing base locale "${baseLocale}", adding fallback bucket`);
        }
        ...
        for (let rowIndex = 1; rowIndex < values.length; rowIndex++) {
            ...
            if (row.length < 3) {
                err(`Sheet "${title}" has row ${rowIndex + 1} with less than 3 base columns, skipping`);
                skippedEmptyRows++;
                continue;
            }
            const label = row[0];
            ...
            for (let columnIndex = 3; columnIndex < header.length; columnIndex++) {
                ...
                const isMissingTranslation = cell == null || (typeof cell === 'string' && cell.trim() === '');
