import { compareStrings, isRecord, log, normalizeMessageMeta, } from './shared.js';
function extractPlaceholderDefinitions(meta) {
    ...
    return Object.entries(meta.placeholders)
        ...
        return Object.assign({ name,
            type }, (definition.example !== undefined
...
function literalUnion(values, fallback = 'string') {
...
function createParamsType(placeholders) {
    if (!placeholders.length) {
        return 'undefined';
    }
    return `{ ${placeholders
        .map((placeholder) => `${JSON.stringify(placeholder.name)}: ${mapPlaceholderType(placeholder.type)}`)
        .join('; ')} }`;
}
function createMessageMethodType(bucket, message) {
...
function createLoadedMessageMethodType(bucket, message) {
    const paramsType = createParamsType(message.placeholders);
    if (paramsType === 'undefined') {
        return '() => string';
    }
...
function pushDocComment(lines, ...description) {
    lines.push('/**');
...
export function logManifestSummary(manifest) {
    ...
    for (const bucket of manifest.bucketNames) {
        ...
        log(`Bucket "${bucket}": locales=[${bucketLocalesList}], keys=${bucketDefinition.keys.length}, placeholders=${countBucketPlaceholders(bucketDefinition)}`);
    }
}
function buildBucketKeysObject(manifest) {
    return Object.fromEntries(manifest.bucketNames.map((bucket) => [
        bucket,
        manifest.bucketDefinitions[bucket].keys,
    ]));
}
function buildMessageMetaObject(manifest) {
    return Object.fromEntries(manifest.bucketNames.map((bucket) => [
        bucket,
        Object.fromEntries(manifest.bucketDefinitions[bucket].messages
...
function pushJsonConst(lines, declaration, value) {
...
export function createTsIndexSource(manifest) {
    ...
    for (const bucket of manifest.bucketNames) {
        lines.push(`  ${JSON.stringify(bucket)}: ${literalUnion(manifest.bucketDefinitions[bucket].keys)};`);
    }
    lines.push('};', '');
    lines.push('export type MessageKey<TBucket extends BucketName> = BucketKeyMap[TBucket];', '');
    ...
    lines.push('  [key: string]: unknown;');
    lines.push('}', '');
    pushJsonConst(lines, 'export const bucketKeys: Record<BucketName, readonly string[]>', bucketKeys);
    lines.push('');
    pushJsonConst(lines, 'export const messageMeta: Record<BucketName, Partial<Record<string, MessageMeta>>>', messageMeta);
    lines.push('');
    ...
    for (const bucket of manifest.bucketNames) {
        ...
        for (const message of manifest.bucketDefinitions[bucket].messages) {
            lines.push(`    ${JSON.stringify(message.key)}: ${createParamsType(message.placeholders)};`);
        }
        lines.push('  };');
    }
    lines.push('};', '');
    lines.push('export type MessageParams<TBucket extends BucketName, TKey extends MessageKey<TBucket>> = BucketMessageParamsMap[TBucket][Extract<TKey, keyof BucketMessageParamsMap[TBucket]>];', '', 'export type LoadedBucketFacadeMap = {');
    for (const bucket of manifest.bucketNames) {
        lines.push(`  ${JSON.stringify(bucket)}: {`);
        for (const message of manifest.bucketDefinitions[bucket].messages) {
            lines.push(`    ${JSON.stringify(message.key)}: ${createLoadedMessageMethodType(bucket, message)};`);
        }
        lines.push('  };');
    }
    ...
    for (const bucket of manifest.bucketNames) {
        lines.push(`  ${JSON.stringify(bucket)}: {`);
        for (const message of manifest.bucketDefinitions[bucket].messages) {
            lines.push(`    ${JSON.stringify(message.key)}: ${createMessageMethodType(bucket, message)};`);
        }
        lines.push('  };');
    }
    lines.push('};', '', 'export const bucketLocales: Record<BucketName, readonly SupportedLocale[]> = {');
    for (const bucket of manifest.bucketNames) {
    ...
    lines.push('}', '');
    pushDocComment(lines, 'Check whether a bucket exists in the generated manifest.');
    lines.push('export function isBucket(bucket: string): bucket is BucketName {');
    lines.push('  return bucketSet.has(bucket);');
    lines.push('}', '');
    pushDocComment(lines, 'Check whether a key exists inside a generated bucket.');
    lines.push('export function isMessageKey<TBucket extends BucketName>(bucket: TBucket, key: string): key is MessageKey<TBucket> {');
    lines.push('  return (bucketKeys[bucket] ?? []).includes(key);');
    lines.push('}', '');
    pushDocComment(lines, 'Read generated metadata for a bucket message key.');
    lines.push('export function getMessageMeta<TBucket extends BucketName, TKey extends MessageKey<TBucket>>(bucket: TBucket, key: TKey): MessageMeta | undefined {');
    ...
    for (const bucket of manifest.bucketNames) {
        ...
        for (const message of manifest.bucketDefinitions[bucket].messages) {
            const paramsType = createParamsType(message.placeholders);
            if (paramsType === 'undefined') {
                lines.push(`        ${JSON.stringify(message.key)}: () => translateLoaded(${JSON.stringify(bucket)}, ${JSON.stringify(message.key)}, dictionary),`);
            }
            else {
                lines.push(`        ${JSON.stringify(message.key)}: (params) => translateLoaded(${JSON.stringify(bucket)}, ${JSON.stringify(message.key)}, dictionary, params),`);
            }
        }
        lines.push('      } as LoadedBucketFacadeMap[TBucket];');
    }
    lines.push('  }');
    lines.push("  throw new Error(`Missing loaded bucket facade for bucket '${bucket}'.`);");
    lines.push('}', '');
    pushDocComment(lines, 'Create synchronous facades for all buckets from preloaded locale dictionaries.');
    lines.push('export function createLoadedLocaleFacade(dictionaries: Record<BucketName, Record<string, unknown>>): LoadedBucketFacadeMap {');
    lines.push('  return {');
    for (const bucket of manifest.bucketNames) {
        lines.push(`    ${JSON.stringify(bucket)}: createLoadedBucketFacade(${JSON.stringify(bucket)}, dictionaries[${JSON.stringify(bucket)}]),`);
    }
    lines.push('  };');
    lines.push('}', '');
    pushDocComment(lines, 'Load all bucket dictionaries for a locale and expose synchronous Dart-like facades.');
    lines.push('export async function loadLocaleFacade(locale: string): Promise<LoadedBucketFacadeMap> {');
    ...
    lines.push('export async function loadLocale(locale: string): Promise<Record<BucketName, Record<string, unknown>>> {');
    lines.push('  const entries = await Promise.all(');
    lines.push('    bucketNames.map(async (bucket): Promise<[BucketName, Record<string, unknown>]> => [bucket, await loadBucket(bucket, locale)])');
    lines.push('  );');
    lines.push('  return Object.fromEntries(entries) as Record<BucketName, Record<string, unknown>>;');
    lines.push('}', '');
    pushDocComment(lines, 'Load a bucket and format a single message for the requested locale.');
    lines.push('export async function translate<TBucket extends BucketName, TKey extends MessageKey<TBucket>>(bucket: TBucket, key: TKey, locale: string, params?: MessageParams<TBucket, TKey>): Promise<string> {');
    lines.push('  const dictionary = await loadBucket(bucket, locale);');
    lines.push('  return translateLoaded(bucket, key, dictionary, params);');
    lines.push('}', '');
    pushDocComment(lines, 'Create an async bucket translator that resolves locale data on demand.');
    lines.push('export function createBucketTranslator<TBucket extends BucketName>(bucket: TBucket, locale: string) {');
    lines.push('  return async <TKey extends MessageKey<TBucket>>(key: TKey, params?: MessageParams<TBucket, TKey>): Promise<string> => translate(bucket, key, locale, params);');
    lines.push('}', '');
    pushDocComment(lines, 'Create an async Dart-like bucket facade that lazy-loads its dictionary once.');
    lines.push('export function createBucketFacade<TBucket extends BucketName>(bucket: TBucket, locale: string): BucketFacadeMap[TBucket] {');
    ...
    for (const bucket of manifest.bucketNames) {
        ...
        for (const message of manifest.bucketDefinitions[bucket].messages) {
            const paramsType = createParamsType(message.placeholders);
            if (paramsType === 'undefined') {
                lines.push(`        ${JSON.stringify(message.key)}: async () => translateLoaded(${JSON.stringify(bucket)}, ${JSON.stringify(message.key)}, await dictionaryPromise),`);
            }
            else {
                lines.push(`        ${JSON.stringify(message.key)}: async (params) => translateLoaded(${JSON.stringify(bucket)}, ${JSON.stringify(message.key)}, await dictionaryPromise, params),`);
            }
        }
        lines.push('      } as BucketFacadeMap[TBucket];');
    }
    lines.push('  }');
    lines.push("  throw new Error(`Missing bucket facade for bucket '${bucket}'.`);");
    lines.push('}', '');
    pushDocComment(lines, 'Create async facades for all generated buckets for a locale.');
    lines.push('export function createLocaleFacade(locale: string): BucketFacadeMap {');
    lines.push('  return {');
    for (const bucket of manifest.bucketNames) {
        lines.push(`    ${JSON.stringify(bucket)}: createBucketFacade(${JSON.stringify(bucket)}, locale),`);
    }
    lines.push('  };');
    lines.push('}', '');
    pushDocComment(lines, 'Load all generated locales and all generated buckets.');
    lines.push('export async function loadLocales(): Promise<Record<SupportedLocale, Record<BucketName, Record<string, unknown>>>> {');
    lines.push('  const entries = await Promise.all(');
    lines.push('    supportedLocales.map(async (locale): Promise<[SupportedLocale, Record<BucketName, Record<string, unknown>>]> => [locale, await loadLocale(locale)])');
    lines.push('  );');
    lines.push('  return Object.fromEntries(entries) as Record<SupportedLocale, Record<BucketName, Record<string, unknown>>>;');
    lines.push('}');
    return lines.join('\n');
}
export function createJsIndexSource(manifest) {
    ...
    lines.push('  const entries = await Promise.all(bucketNames.map(async (bucket) => [bucket, await loadBucket(bucket, locale)]));');
    ...
    lines.push('  return (key, params) => translate(bucket, key, locale, params);');
    lines.push('}', '');
    pushDocComment(lines, 'Create an async Dart-like bucket facade that lazy-loads its dictionary once.');
    ...
    lines.push('  const entries = await Promise.all(supportedLocales.map(async (locale) => [locale, await loadLocale(locale)]));');
    lines.push('  return Object.fromEntries(entries);');
    lines.push('}');
    return lines.join('\n');
}
