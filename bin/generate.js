 ...
 * Copyright (c) 2024 Anton Ustinoff <a.a.ustinoff@gmail.com>
 *
 * Usage:
...
async function generateLocalizationTable(sheets) {
    ...
    for (const { title, values } of sheets) {
        ...
        log(`Sheet "${title}" summary: processed=${processedRows}, skippedEmptyRows=${skippedEmptyRows}, skippedEmptyLabels=${skippedEmptyLabels}`);
    }
    return buckets;
}
/**
...
async function generateIndexTs(outputDir, prefix) {
    ...
    lines.push(`      buckets.map(bucket => ${localesVar}[bucket][locale]().then(m => [bucket, (m as any).default] as const))`);
    lines.push('    );');
